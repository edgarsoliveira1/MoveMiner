...
class Generator(metaclass=abc.ABCMeta):
    ...
    def __init__(self, T: float, dim: int = 2, N: int = 1, dt: float = 1.0, seed: Optional[int] = None):
...
class RandomWalkGenerator(Generator):
    ...
    def __init__(self, T: float, dim: int = 2, N: int = 1, dt: float = 1.0, actions_prob: Optional[np.ndarray] = None,
                 step_length_func: Callable[[Tuple], np.ndarray] = np.ones, seed: Optional[int] = None, **step_length_kwargs):
        ...
        actions_prob = np.tile([1 / 3, 1 / 3, 1 / 3], (dim, 1)) if actions_prob is None else np.asarray(actions_prob, dtype=np.float32)

        if actions_prob.shape != (dim, len(actions)):
            raise ValueError(f"actions_prob must have shape like (dims, {len(actions)})")
    ...
    def _get_position_vectors(self):
        ...
        delta_r = [self.rng.choice(self.actions, p=p, size=(self.n - 1, self.N)) for p in self.actions_prob]
        delta_r = np.swapaxes(delta_r, 0, 1)  # Set time/coordinates as the first/second axis
        delta_r = delta_r * self.step_length  # Scale displacements according to the jump length statistics
        self.r[1:] = np.cumsum(delta_r, axis=0)  # Integrate displacements to get position vectors
        return self.r
