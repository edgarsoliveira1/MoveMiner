...
class DBSCAN:
    def __init__(self, cluster_radius_km = 0.1, min_samples = 1, kms_per_radian = 6371) -> None:
    ...
    def db_scan(self, traj:Trajectory):
        ...
        db = cluster.DBSCAN(eps=self.eps, min_samples=self.min_samples, algorithm='ball_tree', metric='haversine')
        t.gdf[constants.CLUSTER] = -1
        for uid in t.gdf[constants.UID].unique():
            coords = t.gdf[t.gdf[constants.UID] == uid][[constants.LATITUDE, constants.LONGITUDE]].values
